% script name: "test_sync_SEk_Spectral"
%
% We test the "sync_SEk_Spectral" procedure
%
% N.S, Nov 2016
...
estimations = sync_SEk_Spectral( triu(Affin_mat), confidence_weights, d );
...
estimations1 = sync_SEk_Spectral( triu(Affin_mat), confidence_weights, d );
estimations2 = sync_SEk_by_SVD( triu(Affin_mat), ones(n), d );

disp(['outlier in measurements, error with weights: ',num2str(error_calc_SE_k( estimations1, SEk_array))])
disp(['outlier in measurements, error with ones: ',num2str(error_calc_SE_k( estimations2, SEk_array))])
