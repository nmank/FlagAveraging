function [estimations, xcost, info] = sync_LS_SE_d_cost_function_testing(A, W, estimation0, maxiters)
...
for i=1:n
    for j=(i+1):n
        if W(i,j)>0
            ...
            gj = grad.R(:,:,j);
            for p=1:d
                    gi(p,:) = gi(p,:)+ vi'*(mu_i(p,:)*vi);
                    gj(p,:) = gj(p,:)+ vj'*(mu_j(p,:)*vj); 
            end
            grad.R(:,:,i) = gi;
            grad.R(:,:,j) = gj;
            grad.R(:,:,i) = grad.R(:,:,i) - ((b_i-b_ij)*vi');
            grad.R(:,:,j) = grad.R(:,:,j) - (b_j*vj');   
            grad.t(:,j) = grad.t(:,j) + (b_j - mu_j*mu_i'*(b_i-b_ij));
            grad.t(:,i) = grad.t(:,i) - (mu_i*mu_j'*b_j - (b_i-b_ij));
        end
    end
end
