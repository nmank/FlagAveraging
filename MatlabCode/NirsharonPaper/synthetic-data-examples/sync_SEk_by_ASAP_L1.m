function [ estimations ] = sync_SEk_by_ASAP_L1( Affinity_mat, confidence_weights, k, SO_sync_fun )
...
%b_ls = madlin(LS_mat,b,[],[],[],[],[],LS_svd(LS_mat,b));
b_ls = madlin(LS_mat,b);

% [l,t] = size(LS_mat);
% A_mat = [LS_mat, -eye(l)];
% f = [zeros(t,1); ones(l,1)];
% lb = zeros(l+t,1);
% %options = optimoptions('linprog','Algorithm','dual-simplex'); 
% %options.MaxIter = 30;
% %options.x0 = LS_svd(LS_mat,b);
% warning('off')
% %x = linprog(f,A_mat,b,blkdiag(LS_mat,zeros(t,l)),[b;zeros(t,1)],lb,[]);
% [x,fval,exitflag,output] = linprog(f,[],[],A_mat,b,lb,[]);  % with equality const.
% 
% %[x,fval,exitflag,output] = linprog(f,A_mat,b,[],[],lb,[]);
% 
% warning('on')
% b_ls = x(1:t);
% 
% %x = linprog(f,A_mat,b,[],[],lb,[],options);
% norm(LS_mat*LS_svd(LS_mat,b)-b,1)
% norm(LS_mat*b_ls-b,1)
% 
% %b_ls = linprog(A_mat,b,[],[],[],[],[],[],x0);
